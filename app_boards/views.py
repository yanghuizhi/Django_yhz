...
def home(request):
    ...
    return render(request,'app_boards/home.html',{'boards':boards})
...
def board_topics(request,pk):
    ...
    return render(request, 'app_boards/topics.html',
...
def new_topic(request, pk):
    ...
    if request.method == 'POST':
        ...
        form=NewTopicForm()
    return render(request, 'app_boards/new_topic.html',
...
def topic_posts(request, pk, topic_pk):
    ...
    topic.views += 1
    topic.save()
    return render(request, 'app_boards/topic_posts.html',
                  {'topic': topic})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            topic_post_url = '{url}?page={page}#{id}'.format(
                url=topic_url, id=post.pk,
                page=topic.get_page_count())
            return redirect(topic_post_url)
        else:
            form = PostForm()
        return render(request, 'app_boards/reply_topic.html',
...
class PostUpdateView(UpdateView):
    model = Post
    fields = ('message', )
    template_name = 'app_boards/edit_post.html'
...
class BoardListView(ListView):
    ...
    template_name = 'app_boards/home.html'


class TopicListView(ListView):
    model = Topic
    context_object_name = 'topics'
    template_name = 'app_boards/topics.html'
    paginate_by = 20
...
class PostListView(ListView):
    ...
    template_name = 'app_boards/topic_posts.html'
    paginate_by = 2
